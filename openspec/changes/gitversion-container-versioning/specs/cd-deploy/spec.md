...
The CD pipeline SHALL run `az deployment group create` with `infra/main.bicep` to provision or update all Azure resources in an idempotent manner. The image tag parameter SHALL be the semantic version (`fullSemVer`) computed by the `version` job, not the raw git SHA.
...
#### Scenario: Subsequent deployment updates image tags to new SemVer
- **WHEN** the deploy job runs on a subsequent push with a new semantic version
- **THEN** Bicep updates both Container Apps to use the new `<fullSemVer>` image tags
- **THEN** ACA performs a rolling update with zero downtime

### Requirement: Authenticate to Azure via OIDC
The workflow SHALL authenticate to Azure using federated identity (OIDC) with `azure/login` and the secrets `AZURE_CLIENT_ID`, `AZURE_TENANT_ID`, and `AZURE_SUBSCRIPTION_ID` — no long-lived client secret stored in GitHub.
